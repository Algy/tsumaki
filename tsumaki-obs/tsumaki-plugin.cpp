...
struct obs_source_frame* filter_video(void *data, struct obs_source_frame *obs_frame) {
    TsumakiFilter* filter = static_cast<TsumakiFilter *>(data);
...
extern "C" {
    bool obs_module_load(void) {
        tsumaki_filter.id = "tsumaki_filter";
        tsumaki_filter.type = OBS_SOURCE_TYPE_FILTER;
        tsumaki_filter.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_ASYNC;
        tsumaki_filter.get_name = get_name;
        tsumaki_filter.create = create;
        tsumaki_filter.destroy = destroy;
        tsumaki_filter.get_properties = get_properties;
        tsumaki_filter.filter_video = filter_video;
        tsumaki_filter.filter_remove = filter_remove;
